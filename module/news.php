...
$cat = isset($_REQUEST['cat']) ? killInjection($_REQUEST['cat']) : '';
if ($cat == '') {
...
<table align="center" cellSpacing=0 cellPadding=0 width="98%" border=0>
    ...
    if ($p < $i - 1) {
        echo '<a class="aLink3" title="' . $titleNext . '" href="./?frame=' . $_REQUEST['frame'] . '&cat=' . $_REQUEST['cat'] . '&' . $param . '&p=' . ($p + 1) . '">[&gt;]</a> ';
    }
    if ($p < $pages - 1) {
        echo '<a class="aLink3" title="' . $titleLast . '" href="./?frame=' . $_REQUEST['frame'] . '&cat=' . $_REQUEST['cat'] . '&' . $param . '&p=' . ($pages - 1) . '">[&gt;&gt;]</a>';
    }
    echo '</td></tr>';
    ?>
</table><br>
